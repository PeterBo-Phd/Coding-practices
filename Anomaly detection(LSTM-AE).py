...
#Load the data and index settings
dataframe = pd.read_csv('#name#.csv')
df = dataframe[['Date', 'Close']]
df['Date'] = pd.to_datetime(df['Date'])

#plot the line graph of price during the selected time period
sns.lineplot(x=df['Date'], y=df['Close'])

print("Start date is: ", df['Date'].min())
print("End date is: ", df['Date'].max())

#Change train data from period 1 to period 2 as well as the test data
train, test = df.loc[df['Date'] <= 'Year1-month1-day1'], df.loc[df['Date'] > 'Year1-month1-day1']
...
seq_size = 30  # Number of time steps to look back(slide window)
