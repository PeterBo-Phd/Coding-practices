...
class Model(object): 
	...
	def padding(self, data):
		padded_batch_set = []
		max_docs1_len = max([len(doc1) for doc1,doc2,image,anti,label in data])
		max_docs2_len = max([len(doc2) for doc1,doc2,image,anti,label in data])

		max_words1_len = max([max([len(sentence) for sentence in doc1]) for doc1,doc2,image,anti,label in data])
		max_words2_len = max([max([len(sentence) for sentence in doc2]) for doc1,doc2,image,anti,label in data])
		for doc1,doc2,image,anti,label in data:
			docs1_len = len(doc1)
			docs2_len = len(doc2)
			...
			padded_batch_set.append([doc_pad1,doc_pad2,image,anti,label])
			...
			with slim.arg_scope([slim.batch_norm], is_training = is_training):
				with slim.arg_scope([slim.conv2d, slim.max_pool2d, slim.avg_pool2d], padding = 'SAME'):
					...
					with tf.variable_scope('block3'):
						...
						net = self.residual_block(net, 64, 256, stride = 2, scope = 'residual_block2')
						if verbose:
							print('block3:', net.get_shape().as_list())
					with tf.variable_scope('block4'):
						net = self.residual_block(net, 128, 512, stride = 2, scope = 'residual_block1')
						net = self.residual_block(net, 128, 512, stride = 2, scope = 'residual_block2')
...
# DenseNet
	...
	def transition(self, x, out_depth, scope = 'transition', reuse = None):
		...
		with tf.variable_scope(scope, reuse = reuse):
			...
			net = slim.avg_pool2d(net, [2, 2], 2, scope = 'avg_pool', padding = 'SAME')
			return net

	def densenet(self, x, num_classes, growth_rate = 8, block_layers = [2, 4, 4, 2], is_training = None, scope = 'densenet', reuse = None, verbose = False):
		with tf.variable_scope(scope, reuse = reuse):
			with slim.arg_scope([slim.batch_norm], is_training = is_training):
				if verbose:
					print('input: ', x.get_shape().as_list())
				with tf.variable_scope('block0'):
					net = slim.conv2d(x, 32, [7, 7], stride = 1, normalizer_fn = None, activation_fn = None, scope = 'dconv7x7')
					net = slim.batch_norm(net, activation_fn = None, scope = 'bn')
					net = tf.nn.relu(net, name = 'activation')
					net = slim.max_pool2d(net, [2, 2], stride = 2, scope = 'dmax_pool', padding = 'SAME')
					if verbose:
						print('dblock0: ', net.get_shape().as_list())
				for i, num_layers in enumerate(block_layers):
					with tf.variable_scope('block%d' % (i + 1)):
						...
						if i != len(block_layers) - 1:
							...
							net = self.transition(net, current_depth // 2) #除以2并向下取整
					if verbose:
						print('dblock%d: '% (i+1), net.get_shape().as_list())
				with tf.variable_scope('block%d' % (len(block_layers) + 1)):
					net = slim.batch_norm(net, activation_fn = None, scope = 'bn')
					net = tf.nn.relu(net, name = 'activation')
					net = tf.reduce_mean(net, [2, 2], name = 'global_pool', keep_dims = True)
					if verbose:
						print('dblock%d: '% (len(block_layers) + 1), net.get_shape().as_list())
					net = slim.conv2d(net, 128, [7, 1], stride = 7, scope = 'dconv_7x7_2')
					net = slim.avg_pool2d(net, [4, 1], 4, scope = 'dmax_pool')
					if verbose:
						print('dblock%d: '% (len(block_layers) + 1), net.get_shape().as_list())
				with tf.variable_scope('regression'):
					net = slim.flatten(net, scope = 'dflatten')
					net = slim.fully_connected(net, 100, activation_fn = tf.nn.relu, normalizer_fn = None, scope = 'dlogit1')
					net = slim.fully_connected(net, 10, activation_fn = None, normalizer_fn = None, scope = 'dlogit2')				
					net = slim.fully_connected(net, num_classes, activation_fn = None, normalizer_fn = None, scope = 'dlogit3')
					if verbose:
						print('dregression: ', net.get_shape().as_list())
					return net
...
#	
	def build(self, reuse = None):
		...
		self.anti = tf.placeholder(tf.float32, [None, 448, 448, 3])

		self.labelsforCNN = tf.placeholder(tf.float32, [None, 1])
		self.imagesforCNN = tf.placeholder(tf.float32, [None, 448, 448, 3])
# VGG
...
# resnet
		...
		self.is_training1 = tf.placeholder(tf.bool, name = 'is_training')
		with slim.arg_scope([slim.conv2d], activation_fn = tf.nn.relu, normalizer_fn = slim.batch_norm) as sc:
			conv_scope = sc
		with slim.arg_scope(conv_scope):
			cnn_output33 = self.resnet(self.anti, 1, is_training = self.is_training1, verbose = True)
# densenet
		self.is_training2 = tf.placeholder(tf.bool, name = 'is_training')
		with slim.arg_scope([slim.conv2d], activation_fn = tf.nn.sigmoid, normalizer_fn = slim.batch_norm) as sc:
			conv_scope = sc
		with slim.arg_scope(conv_scope):
			cnn_output3 = self.densenet(self.imagesforCNN, 1, is_training = self.is_training2, verbose = True)
		print(cnn_output3.get_shape().as_list()) # [None, 6, 30, 1]
...
# loss
		self.pred_anti = cnn_output33
		self.pred_CNN = cnn_output3+cnn_output33
		self.mean_loss_CNN_anti = tf.reduce_mean(tf.square(self.labelsforCNN - self.pred_anti)) ** 0.5 
		self.mean_loss_CNN = 0.7*tf.reduce_mean(tf.square(self.labelsforCNN - cnn_output3)) ** 0.5 + 0.3*self.mean_loss_CNN_anti
		...
		with tf.Session(config=config) as sess:
			...
			for t in range(epoch):
				...
				for i, batch_sample in enumerate(batch_set):
					_, _, imgs, anti, labels = zip(*batch_sample)
					...
					batch_loss, _,pred, dummy_loss = sess.run([self.mean_loss_CNN, self.train_op_CNN, self.pred_CNN, self.mean_loss_CNN],
											 {self.imagesforCNN: imgs, self.anti: anti, self.labelsforCNN: labelsforCNN, self.dropout_keep_prob: 0.5, self.is_training1: 1, self.is_training2: 1})
	...
	def evaluate(self, test_set, sess): # 此函数处理测试集
		...
		for i, batch_sample in enumerate(batch_set):
			...
			batch_loss, pred = sess.run([self.mean_loss_CNN, self.pred_CNN],
											 {self.imagesforCNN: imgs, self.anti: anti, self.labelsforCNN: labelsforCNN,  self.dropout_keep_prob: 1.0, self.is_training1: 1, self.is_training2: 1, self.reuse: True})
