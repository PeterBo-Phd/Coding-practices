...
class Model(object): 
	...
	def linear1(self, input_, output_size, scope=None):
	    ...
	    return tf.nn.sigmoid(tf.matmul(input_, tf.transpose(matrix1)) + bias_term1)
	...
	def linear6(self, input_, output_size, act = tf.nn.relu, scope = 'fully_connect', reuse = None):
		...
		with tf.variable_scope(scope+'weight2333'):
			weight1 = self.variable_weight([1, 60, 1])
	...
	def build(self):
		self.docs1 = tf.placeholder(tf.float32, [None, 60, 5]) 
		self.docs2 = tf.placeholder(tf.float32, [None, 60, 17]) # 三维，分别是batch size（每个batch多少个样本）、一个样本多少天、一天多少个数据。None处也可改为真实的已知数值。
		...
		docs_embed = (self.linear6(docs_embed, 400, scope = 'fc14')) # 加权则有这一层，30
		print(docs_embed.get_shape().as_list())
		docs_embed = self.linear1(docs_embed, 100, scope = 'fc11')
		...
		self.mean_loss3 = tf.reduce_mean(tf.square(self.labels - self.pred))**0.5

		mean_loss1 = tf.abs((self.labels - self.pred)/self.labels)**0.5
		self.mean_loss2 = tf.reduce_mean(mean_loss1)
		self.mean_loss = self.mean_loss3
		# self.mean_loss = tf.mean_absolute_percentage_error(self.labels, self.pred)
			...
			for j in range(epoch):
				...
				for i, batch_sample in enumerate(batch_set):
					...
					batch_loss, _,pred = sess.run([self.mean_loss, self.train_op, self.pred],
											 {self.docs1: docs1, self.docs2: docs2, self.labels: labels, self.dropout_keep_prob: self.dropout}) # 前面[]内是输出，后面{}内是输入
	...
	def evaluate(self, test_set, sess): # 此函数处理测试集
		...
		for batch_sample in batch_set:
			...
			batch_loss, pred = sess.run([self.mean_loss, self.pred],
			 								 {self.docs1: docs1, self.docs2: docs2, self.labels: labels, self.dropout_keep_prob: 1.0})
			loss += batch_loss
			for m in pred:
				preds.append(m)
		# print(pred)
